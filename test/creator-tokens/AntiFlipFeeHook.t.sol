...
contract AntiFlipFeeHookTest is Test {
    ...
    function test_Setup_HookPermissions() public view {
        ...
        assertFalse(
            permissions.afterAddLiquidityReturnDelta, "afterAddLiquidityReturnDelta should be false"
        );
        assertFalse(
    ...
    function test_Setup_ImmutableVariables() public view {
        ...
        assertLe(
            ...
            block.timestamp,
            "Graduation timestamp should be <= current time"
        );
    }

    function test_Setup_CreatorTokenSupply() public view {
        assertEq(
            creatorToken.totalSupply(), Config.CREATOR_COIN_SUPPLY, "Total supply should be 1M"
        );
        assertGt(creatorToken.balanceOf(creator), 0, "Creator should have tokens");
        assertGt(
            creatorToken.balanceOf(address(vestingWallet)), 0, "Vesting wallet should have tokens"
        );
    }
    ...
    function test_Window_IsWithinRange() public view {
        ...
        uint256 window = AntiFlipFeeLib.calculateWindow(
            user1, address(creatorToken), buyTime, hook.graduationTimestamp()
        );
    ...
    function test_Window_DifferentForDifferentUsers() public view {
        ...
        uint256 window2 = AntiFlipFeeLib.calculateWindow(
            user2, address(creatorToken), buyTime, hook.graduationTimestamp()
        );
    ...
    function test_Window_ChangesWithBuyTimestamp() public view {
        ...
        uint256 firstWindow = AntiFlipFeeLib.calculateWindow(
            user1, address(creatorToken), firstBuyTime, hook.graduationTimestamp()
        );

        uint256 secondWindow = AntiFlipFeeLib.calculateWindow(
            user1, address(creatorToken), secondBuyTime, hook.graduationTimestamp()
        );
    ...
    function test_Window_AllInRange_MultipleScenarios() public view {
        ...
        for (uint256 i = 0; i < buyTimes.length; i++) {
            uint256 window = AntiFlipFeeLib.calculateWindow(
                user1, address(creatorToken), buyTimes[i], hook.graduationTimestamp()
            );
    ...
    function test_FeeDistribution_Tier1_250kTokens() public view {
        ...
        (uint256 foundationBps, uint256 creatorBps) =
            AntiFlipFeeLib.getFeeRates(creator, address(creatorToken), address(vestingWallet));
    ...
    function test_FeeDistribution_Tier4_ReallyUnder50k() public {
        ...
        testVesting = new CreatorVesting(user1, uint64(block.timestamp), 365 days, 30 days);
        ...
        (uint256 foundationBps, uint256 creatorBps) =
            AntiFlipFeeLib.getFeeRates(user1, address(testToken), address(testVesting));
    ...
    function testFuzz_Window_AllUsers(address user, uint256 buyTime) public view {
        ...
        uint256 window = AntiFlipFeeLib.calculateWindow(
            user, address(creatorToken), buyTime, hook.graduationTimestamp()
        );
    ...
    function testFuzz_FeeDistribution_AlwaysValid(uint256 creatorBalance, uint256 vestingBalance)
        public
    {
        ...
        for (uint256 i = 0; i < testAmounts.length; i++) {
            ...
            if (totalFee > 0) {
                (uint256 foundationBps, uint256 creatorBps) = AntiFlipFeeLib.getFeeRates(
                    creator, address(creatorToken), address(vestingWallet)
                );
                ...
                assertEq(
                    foundationFee + creatorFee, totalFee, "Fee split must equal total fee exactly"
                );
            }
        }
    }
    ...
    function _setCreatorHoldings(uint256 targetAmount) internal {
        ...
        console.log(
            "Actual total:",
            (creatorToken.balanceOf(creator) + creatorToken.balanceOf(address(vestingWallet)))
                / 1e18
        );
    }
}
